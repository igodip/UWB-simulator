...
int main(int argc, char ** argv)
{
	...
	Config::SetDefault("ns3::UwbModuleLeApp::PingInterval", TimeValue(MilliSeconds(3.0)));
	Config::SetDefault("ns3::UwbModuleLeApp::EndPhase", TimeValue(MilliSeconds(300.0)));

	//File 
	std::ofstream myfile;
	myfile.open("example.csv");

	uint32_t max_rounds = 5;
	...
	for (uint32_t k = 1; k <= max_rounds; ++k)
	{
		...
		uwbModuleHelper.InstallLeNodes(nodeContainer);
		...
		for (uint32_t i = 0; i < nodeContainer.GetN(); ++i)
		{
			...
			Ptr<UwbModuleLeApp> nodeApp = DynamicCast<UwbModuleLeApp>(netDevice->GetManager());
		...
		for (uint32_t i = 0; i < nodeContainer.GetN(); ++i)
		{
			...
			std::cout << n->GetId() << " ";
			std::cout << n->GetObject<MobilityModel>()->GetPosition() << "  \t" << nodeApp->GetLeader() << std::endl;
